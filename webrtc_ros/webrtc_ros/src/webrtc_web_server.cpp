...
WebrtcWebServer::WebrtcWebServer(rclcpp::Node::SharedPtr nh) : nh_(nh) {}
WebrtcWebServer::~WebrtcWebServer() {}

class WebrtcWebServerImpl : public WebrtcWebServer {
public:
  WebrtcWebServerImpl(rclcpp::Node::SharedPtr nh, int port,
                      SignalingChannelCallback callback, void *data)
      : WebrtcWebServer(nh),
    ...
    std::string webrtc_ros_path =
        ament_index_cpp::get_package_share_directory("webrtc_ros");
    ...
    handler_group_.addHandlerForPath(
        "/list_streams.json",
        std::bind(&WebrtcWebServerImpl::handle_list_streams, this,
    ...
    handler_group_.addHandlerForPath(
        "/.+", async_web_server_cpp::HttpReply::from_filesystem(
                   ...
                   webrtc_ros_path + "/web", false, any_origin_headers));
    server_.reset(new async_web_server_cpp::HttpServer(
        "0.0.0.0", boost::lexical_cast<std::string>(port),
        std::bind(ros_connection_logger, handler_group_, std::placeholders::_1,
...
private:
  ...
  bool handle_list_streams(const async_web_server_cpp::HttpRequest &request,
    ...
    std::string topicPrefix =
        "rt"; // get_topic_names_and_types prepends this; needs to be removed.
    ...
    // Don't use jsoncpp cause we link against c++11 library (many not actually
    // be an issue)
...
WebrtcWebServer *WebrtcWebServer::create(rclcpp::Node::SharedPtr nh, int port,
