...
MessageHandler *
WebrtcRosServer_handle_new_signaling_channel(void *_this,
                                             SignalingChannel *channel) {
  return ((WebrtcRosServer *)_this)
      ->signaling_thread_->Invoke<MessageHandler *>(
          RTC_FROM_HERE,
          rtc::Bind(&WebrtcRosServer::handle_new_signaling_channel,
...
WebrtcRosServer::WebrtcRosServer(rclcpp::Node::SharedPtr nh)
  ...
  int port;
  nh_->get_parameter_or<int>("port", port, 8090);
  ...
  server_.reset(WebrtcWebServer::create(
      nh_, port, &WebrtcRosServer_handle_new_signaling_channel, this));
}
...
MessageHandler *
WebrtcRosServer::handle_new_signaling_channel(SignalingChannel *channel) {
  auto client =
      std::make_shared<WebrtcClient>(nh_, itf_, image_transport_, channel);
...
void WebrtcRosServer::run() { server_->run(); }

void WebrtcRosServer::stop() { server_->stop(); }
