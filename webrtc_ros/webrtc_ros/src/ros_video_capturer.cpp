...
RosVideoCapturer::RosVideoCapturer(const ImageTransportFactory &it,
...
void RosVideoCapturer::Start() { impl_->Start(this); }

void RosVideoCapturer::Stop() { impl_->Stop(); }

void RosVideoCapturer::imageCallback(
    const sensor_msgs::msg::Image::ConstPtr &msg) {
  cv::Mat bgr;
  if (msg->encoding.find("F") != std::string::npos) {
    // scale floating point images
    cv::Mat float_image_bridge =
        cv_bridge::toCvShare(msg, msg->encoding)->image;
  ...
  if (AdaptFrame(bgr.cols, bgr.rows, system_time_us, &out_width, &out_height,
    ...
    // Apparently, the OnFrame() method could not be called from arbitrary
    // threads in ancient times, and there used to be all kinds of shenanigans
    // here to make sure it is called from the original thread, causing a subtle
    // deadlock bug on object destruction.
    //
...
bool RosVideoCapturer::is_screencast() const { return false; }

absl::optional<bool> RosVideoCapturer::needs_denoising() const { return false; }
webrtc::MediaSourceInterface::SourceState RosVideoCapturer::state() const {
  return webrtc::MediaSourceInterface::kLive;
}
bool RosVideoCapturer::remote() const { return false; }

RosVideoCapturerImpl::RosVideoCapturerImpl(const ImageTransportFactory &it,
                                           const std::string &topic,
                                           const std::string &transport)
...
void RosVideoCapturerImpl::Start(RosVideoCapturer *capturer) {
  ...
  sub_ = it_.subscribe(topic_,
                       std::bind(&RosVideoCapturerImpl::imageCallback,
...
void RosVideoCapturerImpl::imageCallback(
    const sensor_msgs::msg::Image::ConstPtr &msg) {
  std::unique_lock<std::mutex> lock(state_mutex_);
  if (capturer_ == nullptr)
    return;
